...
@Component({
  ...
  standalone: true,
  imports: [],
  templateUrl: './medico-barra-solicitado.component.html',
  styleUrl: './medico-barra-solicitado.component.css',
})
export class MedicoBarraSolicitadoComponent {
  chart?: Chart;
  @Input() turnos: Turno[] = [];
  especialista: string[] = [];
  cantidad: number[] = [];
      ...
      tur_especialistas.forEach((espe, index) => {
        this.turnos.forEach((item) => {
          ...
          if (
            ...
            item.estado === Turno.estado_pediente &&
            fecha > fecha1 &&
            fecha <= fecha2
          ) {
            this.cantidad[index]++;
